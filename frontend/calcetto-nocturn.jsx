...
import { RadarChart, Radar, PolarGrid, PolarAngleAxis, ResponsiveContainer,
  AreaChart, Area, XAxis, YAxis, Tooltip, CartesianGrid, BarChart, Bar } from "recharts";
...
function tier(ovr) {
  if (ovr >= 88) return { label:"LEGGENDA", color:C.gold,  cls:"legenda", glow:`0 0 28px ${C.gold}60` };
  if (ovr >= 78) return { label:"ELITE",    color:C.em,    cls:"elite",   glow:`0 0 20px ${C.emMid}` };
  if (ovr >= 70) return { label:"SILVER",   color:"#7ba8d0",cls:"silver", glow:`0 0 12px #7ba8d040` };
    ...
    <div className="card-player" style={{ animationDelay: `${delay}ms`, ...style }}>
      <div className="card-inner noise scan-line" style={{
        ...
        <div style={{ marginBottom:small?4:6 }}>
          <div style={{ fontFamily:"Oswald,sans-serif", fontSize:small?58:70, fontWeight:700, color:acc,
        ...
        <div style={{ display:"flex", justifyContent:"center", marginBottom:small?6:10 }}>
          <div style={{ position:"relative" }}>
            <div style={{ width:small?60:72, height:small?60:72, borderRadius:"50%", overflow:"hidden",
        ...
        <div style={{ textAlign:"center", marginBottom:small?6:8 }}>
          <div style={{ fontFamily:"Oswald,sans-serif", fontSize:small?15:17, fontWeight:600, color:C.t1,
            letterSpacing:.4, lineHeight:1, textTransform:"uppercase" }}>{player.cognome}</div>
        ...
        {/* Stats strip */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:small?2:4,
...
function Sidebar({ page, setPage, totals }) {
  return (
    <div style={{ width:216, minHeight:"100vh", background:C.surface, borderRight:`1px solid ${C.border}`,
      ...
      <div style={{ padding:"22px 8px 20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:34, height:34, borderRadius:9, background:C.emDim, border:`1px solid ${C.emMid}`,
  ...
  return (
    <div className="page">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
        {statCards.map((s,i) => (
          <div key={i} className="glass tr" style={{ padding:"18px 16px",
      ...
      {/* Top 5 mini-cards */}
      <div className="glass" style={{ padding:20 }}>
        <div style={{ fontSize:10, color:C.t2, fontWeight:700, letterSpacing:2, marginBottom:18, textTransform:"uppercase" }}>Top 5 — Classifica Overall</div>
        <div style={{ display:"flex", gap:16, justifyContent:"center", flexWrap:"wrap" }}>
          {enriched.slice(0,5).map((p, i) => {
            const t = tier(p.ovr);
            const photo = photos[p.id];
            const initials = p.cognome[0] + (p.nome[0]||"");
            return (
              <div key={p.id} style={{ textAlign:"center", animationDelay:`${i*50}ms` }}>
                {i===0 && <div style={{ fontSize:16, marginBottom:4 }}>👑</div>}
                <div style={{ width:54, height:54, borderRadius:"50%", overflow:"hidden",
                  border:`2.5px solid ${t.color}`, background:C.surface2,
                  display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 6px",
                  boxShadow:t.glow }}>
                  {photo ? <img src={photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                         : <span style={{fontFamily:"Oswald",fontSize:18,fontWeight:700,color:t.color}}>{initials}</span>}
                </div>
                <div style={{ fontFamily:"Oswald,sans-serif", fontSize:24, fontWeight:700, color:t.color, lineHeight:1 }}>{p.ovr}</div>
                <div style={{ fontSize:12.5, fontWeight:600, color:C.t1, marginTop:2 }}>{p.cognome}</div>
                <div style={{ fontSize:10.5, color:C.t2 }}>{p.ga_pp.toFixed(2)} G+A/P</div>
              </div>
            );
          })}
        </div>
    ...
    <div style={{ marginBottom:10 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
        <div style={{ fontFamily:"Oswald,sans-serif", fontSize:17, fontWeight:600,
          color: p1wins ? C.em : C.t2, minWidth:50 }}>{fmt(v1)}{suffix}</div>
        ...
        <div style={{ fontFamily:"Oswald,sans-serif", fontSize:17, fontWeight:600,
          color: !p1wins ? C.blue : C.t2, textAlign:"right", minWidth:50 }}>{fmt(v2)}{suffix}</div>
      </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:20, alignItems:"center", marginBottom:24 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
              ...
              <div className="glass2" style={{ padding:"12px 16px", textAlign:"center", width:110 }}>
                <div style={{ fontFamily:"Oswald,sans-serif", fontSize:32, fontWeight:700,
  ...
  return (
    <div className="page">
      ...
      <div className="glass" style={{ overflow:"auto" }}>
        {/* Head */}
        <div style={{ display:"grid", gridTemplateColumns:gc, padding:"10px 16px",
          ...
          {COLS.map(c => (
            <div key={c.k} className="sort-col" onClick={() => { if(key===c.k) setDir(d=>d==="desc"?"asc":"desc"); else{setKey(c.k);setDir("desc");} }}
              style={{ fontSize:9.5, color:key===c.k?C.em:C.t2, fontWeight:700, letterSpacing:.8,
  ...
  return (
    <div className="page">
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {filtered.map((m, i) => (
          <div key={i} className="glass" style={{ padding:"14px 18px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap",
...
export default function App() {
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"DM Sans,sans-serif" }}>
      ...
      {/* Ambient glow */}
      <div style={{ position:"fixed", top:"-20%", left:"30%", width:"500px", height:"500px",
        background:`radial-gradient(ellipse at center, ${C.em}08 0%, transparent 70%)`,
        pointerEvents:"none", zIndex:0 }} />
      <div style={{ position:"fixed", bottom:"-10%", right:"10%", width:"400px", height:"400px",
